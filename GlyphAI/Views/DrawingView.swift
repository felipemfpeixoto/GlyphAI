                ...
                ForEach(0..<16, id: \.self) { row in
                    HStack(spacing: 0) {
                        ForEach(0..<16, id: \.self) { column in
                            Rectangle()
                                .fill(self.grid[row][column] ? Color.black : drawGrid ? Color.ourLightGray : .white)
...
#Preview {
    DrawingView(grid: .constant(Array(repeating: Array(repeating: false, count: 16), count: 16)), lapis: true, drawGrid: true)
}
